...
def create_advanced_feature_vector(sequence, scaler_frames, pca):
    """
    THE KEY UPGRADE: Creates a feature vector with temporal information
    (start, middle, end points) in addition to statistical summaries.
    """
    seq_scaled = scaler_frames.transform(sequence)
    seq_pca = pca.transform(seq_scaled)
    
    # Statistical features (as before)
    mean=seq_pca.mean(axis=0); std=seq_pca.std(axis=0)
    
    # --- NEW TEMPORAL FEATURES ---
    # Capture the start, middle, and end of the motion's trajectory
    start_pos = seq_pca[0]
    mid_pos = seq_pca[len(seq_pca) // 2]
    end_pos = seq_pca[-1]
    ...
    if seq_pca.shape[0] > 1:
        ...
        v_mean=np.zeros(pca.n_components_); v_std=np.zeros(pca.n_components_)
        
    # Combine statistical and temporal features
...
def main():
    ...
    print("\n[PHASE 3] Creating Advanced Feature Dataset...")
    X_features = np.array([create_advanced_feature_vector(seq, scaler_frames, pca) for seq in sequences])
    ...
    print(f"\n[PHASE 4] Training Final Tuned XGBoost Model...")
    ...
    clf = xgb.XGBClassifier(
        ...
        n_estimators=300, learning_rate=0.05, max_depth=6, subsample=0.7, 
        colsample_bytree=0.7, gamma=0.1, random_state=42
    )
    clf.fit(X_train, y_train)
    
    print("\n[PHASE 5] Evaluating and Saving Final Model...")
    y_pred = clf.predict(X_test)
    accuracy = accuracy_score(y_test, y_pred)
    print(f"✅ Final model trained. Validation Accuracy: {accuracy:.4f}")
    ...
    print(f"\n🎉 Successfully saved final, intelligent pipeline to: {OUTPUT_MODEL_PATH}")
